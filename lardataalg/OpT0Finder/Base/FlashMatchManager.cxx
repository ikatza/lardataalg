...
namespace flashana {
  ...
  void FlashMatchManager::Configure(const fhicl::ParameterSet& main_cfg)
  {
    /*
    if (!cfg_file.empty()) {
    ...
    */
    auto const& mgr_cfg = main_cfg.get<fhicl::ParameterSet>(Name());
    
    _allow_reuse_flash = mgr_cfg.get<bool>("AllowReuseFlash");
    _verbosity = (msg::MSGLevel_t)(mgr_cfg.get<unsigned int>("Verbosity"));

    auto const& pmt_pos_cfg = main_cfg.get<fhicl::ParameterSet>("PMT_POSITION");
    ...
    auto const& detector_boundary_cfg = main_cfg.get<fhicl::ParameterSet>("ACTIVE_VOLUME");
    ...
    if (_alg_flash_filter) {
      ...
      _alg_flash_filter->Configure(main_cfg.get<fhicl::ParameterSet>(_alg_flash_filter->AlgorithmName()));
    ...
    if (_alg_tpc_filter) {
      ...
      _alg_tpc_filter->Configure(main_cfg.get<fhicl::ParameterSet>(_alg_tpc_filter->AlgorithmName()));
    ...
    if (_alg_match_prohibit) {
      ...
      _alg_match_prohibit->Configure(main_cfg.get<fhicl::ParameterSet>(_alg_match_prohibit->AlgorithmName()));
    ...
    if (_alg_flash_hypothesis) {
      ...
      _alg_flash_hypothesis->Configure(main_cfg.get<fhicl::ParameterSet>(_alg_flash_hypothesis->AlgorithmName()));
    ...
    if (_alg_flash_match) {
      ...
      _alg_flash_match->Configure(main_cfg.get<fhicl::ParameterSet>(_alg_flash_match->AlgorithmName()));
    ...
    for (auto& custom_alg : _custom_alg_v) {
      ...
      custom_alg->Configure(main_cfg.get<fhicl::ParameterSet>(custom_alg->AlgorithmName()));
...
//    std::cout << "\nIN FlashMatchManager::Match()";
    ...
    if (!_configured)
      ...
      throw OpT0FinderException("Call Configure() before Match()!");
